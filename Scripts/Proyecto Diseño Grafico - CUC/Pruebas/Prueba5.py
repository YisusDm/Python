...
def calcular_longitud_entidades(entidades):
    ...
    for entidad in entidades:
        if entidad.dxftype() == 'LWPOLYLINE':
            ...
            print(f"Linea {line}")
            print(f"Longitud de linea {line.length}")
...
def analizar_dibujo(dxf_file_path):
    ...
    msp = doc.modelspace()
    longitudes_muros = {"Muro 10cm": 0, "Muro 15cm": 0}

    for entidad in msp.query(f'*[layer=="{Muro10}" | layer=="{Muro15}"]'):
        nombre_capa = entidad.get_dxf_attrib('layer')
        longitud_real = calcular_longitud_entidades([entidad])

        if nombre_capa == Muro10:
            longitud_metros = longitud_real * escala_muro_10cm
            longitudes_muros["Muro 10cm"] += longitud_metros
        elif nombre_capa == Muro15:
            longitud_metros = longitud_real * escala_muro_15cm
            longitudes_muros["Muro 15cm"] += longitud_metros

    return longitudes_muros

if __name__ == "__main__":
    archivo_dxf = r"REFPA.dxf"  # Ruta completa de tu archivo DXF
