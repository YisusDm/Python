...
def crea_tablero(fil, col, val):
    #La funcion recibe las variables fil y col, las cuales definen el tamaño de la matriz(tablero)
    #val son los valores con cuales se rellenan
    cabezera = []
    for i in range(col+1):#Crea la indexacion las columnas
        cabezera.append(i)
    tablero=[]
    for i in range(fil):
        tablero.append([i+1])#Crea la indexacion las filas
        for j in range(col):
...
def coloca_pistas(tablero, fil, col):
    for y in range(1,fil+1):
        for x in range(1,col+1):
            if tablero[y][x] == "Ω":
                for i in [-1,0,1]:#Verifica las casillas alrededor
                    for j in [-1,0,1]:
